...
import { recordInitialProduct } from '../services/invRecordService.js';
/**
...
export const createProduct = catchAsync(async (req, res, next) => {
  ...
  const InventoryRecord = await recordInitialProduct(record);
...
export const updateProduct = catchAsync(async (req, res, next) => {
  ...
  if (
    ...
    if (numericQuantity !== undefined) {
      const inv = await InventoryService.getInventoryByProductId(
        updatedProduct._id
      );
      ...
      const record = await recordInitialProduct(rec);
      if (!record)
        return next(new AppError('Record inventory quantity failed.', 400));
