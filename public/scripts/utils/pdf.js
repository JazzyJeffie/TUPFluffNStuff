...
export const generateInventoryPDF = (reportData, fromDate, toDate) => {
  ...
  const safe = (val, fallback = 'N/A') => {
    if (val === undefined || val === null || val === '') return fallback;
    if (!isNaN(Number(val))) return Math.abs(Number(val)); // 🔹 always positive
    return val;
  };

  const safeMoney = val =>
    Math.abs(Number(val?.$numberDecimal ?? val ?? 0)).toLocaleString();

  // Logo
  const imgEl = document.getElementById('logo');
  if (imgEl) {
    const scaleFactor = 4; // 🔹 higher = sharper (try 3–4)
    const canvas = document.createElement('canvas');
    ...
    // Get higher-resolution PNG
    const imgData = canvas.toDataURL('image/png', 1.0);

    // 🔹 Display at consistent size (60x60 points in PDF)
    doc.addImage(imgData, 'PNG', margin, currentY, 60, 60);
  }
  ...
  const totalQuantitySold = reportData.summary?.totalQuantitySold || 0;
  const totalRefunds = reportData.summary?.totalRefunds || 0;

  doc.text(`Total Inventory Value: ${totalInventoryValue}`, margin, currentY);
  currentY += 12;
  doc.text(`Total Quantity Sold: ${totalQuantitySold}`, margin, currentY);
  currentY += 12;
  doc.text(`Total Refunds: ${totalRefunds}`, margin, currentY);
  currentY += 30;
  ...
  // Current Inventory (keep on first page with summary)
  if (reportData.currentInventory?.length) {
    addTable(
      'Current Inventory',
      [
      ...
      reportData.currentInventory.map(item => ({
        ...
        correction: safe(item.correction, 0),
        restocked: safe(item.restocked, 0),
        customerRequest: safe(item.customerRequest, 0),
      })),
      {},
      false // first table stays with summary
    );
  }

  // Manual Adjustments
  if (reportData.manual?.length) {
    addTable(
      'Manual Adjustments',
      ['#', 'Date', 'Product', 'Previous Qty', 'Current Qty', 'Reason', 'Note'],
      reportData.manual.map(m => ({
        date: m.date ? new Date(m.date).toLocaleDateString() : 'N/A',
        product: safe(m.product),
        previousQty: safe(m.previousQuantity, 0),
        currentQty: safe(m.currentQuantity, 0),
  ...
  // Inventory Breakdown
  if (reportData.inventoryBreakdown?.length) {
    addTable(
      'Inventory Breakdown',
      ['#', 'Date', 'Product', 'Previous Qty', 'Current Qty'],
      reportData.inventoryBreakdown.map(b => ({
        date: b.date ? new Date(b.date).toLocaleDateString() : 'N/A',
        product: safe(b.product),
        previousQty: safe(b.previousQuantity, 0),
        currentQty: safe(b.currentQuantity, 0),
      }))
    );
  }
