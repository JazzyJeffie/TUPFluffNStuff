...
const deleteTransactionItem = async id => {
  ...
  if (items.length > 0) {
    await TransactionItem.updateMany(
      { transactionId: id, isDeleted: false },
      { isDeleted: true }
    );
  }

  return items; // ✅ array you can .map on
};
...
const refundItem = async (data, reason) => {
  ...
  try {
    ...
    if (restockReasons.includes(reason.toLowerCase())) {
      const inv = await inventoryModel.findOne({ productId: data.productId });

      if (!inv) throw new Error('Item not found', 404);

      const rec = await inventoryRecord.create({
        productId: data.productId,
        inventoryId: inv._id,
      ...
      if (!rec) throw new Error('Record not saved', 400);
