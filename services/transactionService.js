...
const createTransaction = async ({
  ...
  try {
    ...
    // 3) create transaction items & update inventory
    const txItems = [];
    let discountRatio = 1;
    if (totals.grossAmount > 0) {
      discountRatio = totals.totalAmount / totals.grossAmount; // scale factor
    ...
    for (const item of items) {
      ...
      txItems.push({
        ...
        quantity: qty,
        price: toD128(price),
        totalAmount: toD128(grossLine),
        vatType,
        vatAmount: toD128(vatAmount),
        netAmount: toD128(discountedLine),
      });

      // --- Update inventory and log previous & latest quantity ---
      const inventory = await Inventory.findOneAndUpdate(
        { productId: item._id || item.productId, status: 'active' },
        { $inc: { quantity: -qty }, updatedAt: Date.now() },
        { session, new: true }
          ...
          [
            {
              ...
              prevQuantity: inventory.quantity + qty, // previous quantity before decrement
              quantity: inventory.quantity, // latest quantity
...
const getTransactionHistory = async (filters = {}) => {
  ...
  // ✅ Always populate cashier safely
  let transactions = await Transaction.find(query)
    .sort({ createdAt: -1 })
    .populate({
      path: 'cashier',
      select: 'name',
      options: { lean: true }, // lean even on populated doc
    })
    .lean();

  // ✅ Helper function to safely format cashier name
  const formatCashierName = cashier => {
    if (!cashier?.name) return '[Unknown User]';
    const { firstName = '', middleName = '', lastName = '' } = cashier.name;
    return `${firstName} ${
  ...
  if (filters.user) {
    ...
    transactions = transactions.filter(tx => {
      const fullName = formatCashierName(tx.cashier).toLowerCase();
